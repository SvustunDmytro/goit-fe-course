...
const getUsersByEyeColor = (users, color) => {
  let result = users.filter(el => el.eyeColor === color);
  return result;
};
...
const getUsersByGender = (users, gender) => {
  let result = users.filter(el => el.gender === gender).map(el => el.name);
  return result;
};
...
const getInactiveUsers = users => {
  let result = users.filter(el => !el.isActive);
  return result;
};
...
const getUserByEmail = (users, email) => {
  let result = users.find(el => el.email === email);
  return result;
};
...
const getUsersWithAge = (users, min, max) => {
  let result = users.filter(el => el.age >= min && el.age <= max);
  return result;
};
...
const getTotalBalance = users => {
  let result = users.reduce((acc, el) => acc + el.balance, 0);
  return result; 
};
...
const getUsersByFriend = (users, name) => {
  let result = users.filter(el => el.friends.includes(name)).map(el => el.name);
  return result;
};
